...
def train():
    ...
    for i in range(C):
        ...
        next_err_drv_layer1_in = np.zeros(16)
        for j in range(8):
            ...
            e_drv_layer1_in = (next_err_drv_layer1_in.dot(W_HH.T) + e_drv_layer2_in.dot(W_HY.T))*sigmoid_drv(layer_1)
            next_err_drv_layer1_in = e_drv_layer1_in
        ...
        if i % 100 == 0:
            ...
            print('similarity:', sm)
            print('=====')
    with codecs.open('rnn_rs.txt', mode='w', encoding='utf-8') as file:
        file.write(s)
